...
/**
  * The FullHist class extends Workflow.
  * For this workflow, the ingestion will be splitted by a timestamp (YYYYMM).
  * So if the ingestion concern the current timestamp, old data will be overwrite. However, if it's a new timestamp, data will be appended.
  *
  ...
  */
class FullHist(fs: FileSystem, sc: SparkContext, spark: SparkSession) extends Workflow {
  ...
  def this(fs: FileSystem, fileStatus: FileStatus, sc: SparkContext, spark: SparkSession, singleFilename: String, config: Config) {
    ...
    tasks ++= List(
      ...
      new CreateHiveTable(spark = spark),
