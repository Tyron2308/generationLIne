...
/**
  * The Append class extends Workflow.
  * For this workflow, old data will be appended.
  *
  ...
  */
class Append(fs: FileSystem, sc: SparkContext, spark: SparkSession) extends Workflow {
  ...
  def this(fs: FileSystem, fileStatus: FileStatus, sc: SparkContext, spark: SparkSession, singleFilename: String, config: Config) {
    ...
    tasks ++= List(
      ...
      new InsertIntoHive(spark = spark, storage = Storage.VALID, mode = SaveMode.Append),
